...
interface OsirisMapProps {
  ...
  /** PAQSHI feed-merge (R2-16): active Paqshi intel-layer ids to render. */
  paqshiDataLayers?: string[];
...
function OsirisMap({ data, activeLayers, onEntityClick, onMouseCoords, onRightClick, onViewStateChange, flyToLocation, projection = 'globe', mapStyle = 'dark', sweepData, scanTargets = [], demoMode = false, theme = 'core', drawnPolygons = [], arcgisLayers = [], drawMode = null, onDrawComplete, onDrawProgress, onDrawCancel, drawCommand = null, onMapCenter, route = null, userLocation = null, followUser = false, onFollowInterrupt, navigating = false, aircraftAirports = {}, paqshiDataLayers = [] }: OsirisMapProps) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      // ── PAQSHI intelligence fusion layer (R2-16): live chokepoint stress ──
      // Paqshi's own signal on the globe — a stress-graded heat dot per
      // chokepoint (green→amber→red by stress_level), fed by /api/paqshi/*.
      const PQ_STRESS_COLOR: any = ['interpolate', ['linear'], ['get', 'stress'],
        0.0, '#2ea043', 0.4, '#d4a017', 0.6, '#e3873a', 0.8, '#f85149', 1.0, '#ff2d2d'];
      // Cascade arcs (under the dots) — colored by the strongest endpoint stress.
      map.addSource('paqshi-arcs', { type: 'geojson', data: EMPTY_FC });
      map.addLayer({ id: 'paqshi-arc-lines', type: 'line', source: 'paqshi-arcs',
        layout: { 'line-cap': 'round' },
        paint: {
          // hl=1 → this arc touches the selected node (bright cyan-blue);
          // dim=1 → a selection exists elsewhere (fade right down).
          'line-color': ['case', ['==', ['get', 'hl'], 1], '#58a6ff',
            ['interpolate', ['linear'], ['get', 'stress'], 0, '#3b6ea5', 0.5, '#d4a017', 0.8, '#f85149']],
          'line-width': ['case', ['==', ['get', 'hl'], 1], 2.6,
            ['interpolate', ['linear'], ['get', 'weight'], 0, 0.5, 1, 2.2]],
          'line-opacity': ['case', ['==', ['get', 'dim'], 1], 0.03,
            ['case', ['==', ['get', 'hl'], 1], 0.9,
              ['interpolate', ['linear'], ['get', 'stress'], 0, 0.14, 1, 0.5]]],
          'line-blur': 0.4,
        } });
      // Chokepoint markers — bigger + brighter than the first pass so they READ
      // as the primary Paqshi signal (the user: "chokepoints don't show visibly").
      map.addSource('paqshi-cp', { type: 'geojson', data: EMPTY_FC });
      map.addLayer({ id: 'paqshi-cp-glow', type: 'circle', source: 'paqshi-cp', paint: {
        'circle-radius': ['interpolate', ['linear'], ['get', 'stress'], 0, 11, 1, 40],
        'circle-color': PQ_STRESS_COLOR,
        'circle-opacity': ['case', ['==', ['get', 'dim'], 1], 0.04, 0.20], 'circle-blur': 1,
      } });
      map.addLayer({ id: 'paqshi-cp-ring', type: 'circle', source: 'paqshi-cp',
        filter: ['>=', ['get', 'stress'], 0.6],
        paint: {
          'circle-radius': ['interpolate', ['linear'], ['get', 'stress'], 0.6, 10, 1, 18],
          'circle-color': 'rgba(0,0,0,0)',
          'circle-stroke-width': 1.5, 'circle-stroke-color': PQ_STRESS_COLOR,
          'circle-stroke-opacity': ['case', ['==', ['get', 'dim'], 1], 0.12, 0.7],
        } });
      map.addLayer({ id: 'paqshi-cp-dot', type: 'circle', source: 'paqshi-cp', paint: {
        // selected node gets a bit bigger; dimmed nodes fade to a faint marker.
        'circle-radius': ['+', ['interpolate', ['linear'], ['get', 'stress'], 0, 5, 1, 11],
          ['case', ['==', ['get', 'sel'], 1], 2, 0]],
        'circle-color': PQ_STRESS_COLOR,
        'circle-opacity': ['case', ['==', ['get', 'dim'], 1], 0.28, 1],
        'circle-stroke-width': ['case', ['==', ['get', 'sel'], 1], 2.4, 1.6],
        'circle-stroke-color': ['case', ['==', ['get', 'sel'], 1], '#58a6ff', 'rgba(255,255,255,0.85)'],
        'circle-stroke-opacity': ['case', ['==', ['get', 'dim'], 1], 0.25, 1],
      } });
      map.addLayer({ id: 'paqshi-cp-label', type: 'symbol', source: 'paqshi-cp',
        // show a label for hot CPs OR any node in the current selection set.
        filter: ['any', ['>=', ['get', 'stress'], 0.45], ['==', ['get', 'sel'], 1]],
        layout: { 'text-field': ['get', 'name'], 'text-size': 11, 'text-offset': [0, 1.4],
          'text-anchor': 'top', 'text-font': ['Open Sans Bold'], 'text-max-width': 12,
          'text-optional': true },
        paint: { 'text-color': ['case', ['==', ['get', 'sel'], 1], '#8cc2ff', '#e6edf3'],
          'text-opacity': ['case', ['==', ['get', 'dim'], 1], 0.25, 1],
          'text-halo-color': '#04040a', 'text-halo-width': 1.4 } });
  ...
  // ── PAQSHI fusion layer (R2-16): fetch chokepoint stress + click popups ──
  useEffect(() => {
    if (!mapReady) return;
    const map = mapRef.current;
    if (!map) return;
    let cancelled = false;
    // Selection state (closure-local — persists across refreshes & handlers).
    let selected: string | null = null;
    let cpFeats: any[] = [];
    let arcFeats: any[] = [];
    const adjacency: Record<string, Set<string>> = {};   // cp id → directly linked ids

    // Re-paint the CP + arc sources with dim/hl/sel flags for the current
    // selection. Selecting a node lights it + its direct connections (and their
    // labels) and fades everything else — the "focus on one node" ask.
    const applySelection = () => {
      const cpSrc = map.getSource('paqshi-cp') as any;
      const arcSrc = map.getSource('paqshi-arcs') as any;
      const nbr = selected ? (adjacency[selected] || new Set<string>()) : null;
      const cpOut = cpFeats.map((f) => {
        const id = f.properties.id;
        const inSet = !selected || id === selected || (nbr ? nbr.has(id) : false);
        return { ...f, properties: { ...f.properties,
          sel: selected && id === selected ? 1 : (selected && inSet ? 1 : 0),
          dim: selected && !inSet ? 1 : 0 } };
      });
      const arcOut = arcFeats.map((f) => {
        const { from, to } = f.properties;
        const touches = selected && (from === selected || to === selected);
        return { ...f, properties: { ...f.properties,
          hl: touches ? 1 : 0, dim: selected && !touches ? 1 : 0 } };
      });
      if (cpSrc) cpSrc.setData({ type: 'FeatureCollection', features: cpOut });
      if (arcSrc) arcSrc.setData({ type: 'FeatureCollection', features: arcOut });
    };

    const load = async () => {
      try {
        const [cpRes, relRes] = await Promise.all([
          fetch('/api/paqshi/chokepoints'),
          fetch('/api/paqshi/relations?cap=130&min_weight=0.5').catch(() => null),
        ]);
        const j = await cpRes.json();
        if (cancelled) return;
        const cps = (j.chokepoints || []).filter((c: any) => c.lat && c.lng);
        const coord: Record<string, [number, number]> = {};
        const stressById: Record<string, number> = {};
        cps.forEach((c: any) => { coord[c.id] = [c.lng, c.lat]; stressById[c.id] = c.stress; });
        cpFeats = cps.map((c: any) => ({
          type: 'Feature' as const,
          geometry: { type: 'Point' as const, coordinates: [c.lng, c.lat] },
          properties: { id: c.id, name: c.name, stress: c.stress, baseline: c.baseline,
            criticality: c.criticality, category: c.category, country: c.country },
        }));

        // Cascade arcs — bow scaled to the endpoint distance and offset
        // PERPENDICULAR to the segment (a fixed +2.5° latitude bow made short,
        // vertically-aligned pairs shoot absurdly high — the Istanbul artifact).
        let relJson: any = { relations: [] };
        try { if (relRes) relJson = await relRes.json(); } catch { /* */ }
        Object.keys(adjacency).forEach((k) => delete adjacency[k]);
        arcFeats = (relJson.relations || [])
          .filter((e: any) => coord[e.from] && coord[e.to] && e.from !== e.to)
          .map((e: any) => {
            const a = coord[e.from], b = coord[e.to];
            (adjacency[e.from] ||= new Set()).add(e.to);
            (adjacency[e.to] ||= new Set()).add(e.from);
            const dx = b[0] - a[0], dy = b[1] - a[1];
            const dist = Math.hypot(dx, dy) || 1;
            const bow = Math.min(3.2, dist * 0.16);          // proportional, capped
            const px = -dy / dist, py = dx / dist;            // unit perpendicular
            const mid: [number, number] = [(a[0] + b[0]) / 2 + px * bow, (a[1] + b[1]) / 2 + py * bow];
            const stress = Math.max(stressById[e.from] || 0, stressById[e.to] || 0);
            return { type: 'Feature' as const,
              geometry: { type: 'LineString' as const, coordinates: [a, mid, b] },
              properties: { from: e.from, to: e.to, weight: e.weight, type: e.type, stress } };
          });
        // if the selected node vanished from the refresh, clear selection
        if (selected && !coord[selected]) selected = null;
        applySelection();
      } catch { /* backend may be down — layers just stay empty */ }
    };
    load();
    ...
    const onClick = (e: any) => {
      ...
      // toggle selection focus on this node
      selected = (selected === p.id) ? null : p.id;
      applySelection();
      const s = Number(p.stress) || 0;
      const col = s >= 0.8 ? '#f85149' : s >= 0.55 ? '#e3873a' : s >= 0.4 ? '#d4a017' : '#2ea043';
      const nConn = (adjacency[p.id]?.size) || 0;
      new maplibregl.Popup({ closeButton: true, maxWidth: '300px' })
        .setLngLat(e.lngLat)
        .setHTML(
          `<div style="font-family:ui-monospace,monospace;color:#e6edf3;min-width:190px;background:rgba(10,12,22,0.96);border:1px solid ${col}55;border-left:3px solid ${col};border-radius:8px;padding:12px 14px;box-shadow:0 8px 30px rgba(0,0,0,0.6)">
            <div style="font-size:9px;letter-spacing:.18em;color:#58a6ff;text-transform:uppercase;margin-bottom:5px">PAQSHI · CHOKEPOINT</div>
            <div style="font-size:13px;font-weight:700;margin-bottom:8px;color:#fff">${(p.name || p.id || '')}</div>
            <div style="display:flex;justify-content:space-between;font-size:11px;margin:3px 0"><span style="color:#8b949e">Stress</span><span style="color:${col};font-weight:700">${(s * 100).toFixed(0)}%</span></div>
            <div style="display:flex;justify-content:space-between;font-size:11px;margin:3px 0"><span style="color:#8b949e">Criticality</span><span>${((Number(p.criticality) || 0) * 100).toFixed(0)}%</span></div>
            <div style="display:flex;justify-content:space-between;font-size:11px;margin:3px 0"><span style="color:#8b949e">Category</span><span>${p.category || '—'}</span></div>
            <div style="display:flex;justify-content:space-between;font-size:11px;margin:3px 0"><span style="color:#8b949e">Country</span><span>${p.country || '—'}</span></div>
            <div style="display:flex;justify-content:space-between;font-size:11px;margin:3px 0"><span style="color:#8b949e">Cascade links</span><span style="color:#58a6ff">${nConn}</span></div>
          </div>`)
        .addTo(map);
    };
    // click on empty ocean/land (not a CP dot) clears the selection focus
    const onMapClick = (e: any) => {
      const hits = map.queryRenderedFeatures(e.point, { layers: ['paqshi-cp-dot'] });
      if ((!hits || hits.length === 0) && selected) { selected = null; applySelection(); }
    };
    const enter = () => { map.getCanvas().style.cursor = 'pointer'; };
    const leave = () => { map.getCanvas().style.cursor = ''; };
    map.on('click', 'paqshi-cp-dot', onClick);
    map.on('click', onMapClick);
    map.on('mouseenter', 'paqshi-cp-dot', enter);
    map.on('mouseleave', 'paqshi-cp-dot', leave);
    return () => {
      cancelled = true; clearInterval(iv);
      map.off('click', 'paqshi-cp-dot', onClick);
      map.off('click', onMapClick);
      map.off('mouseenter', 'paqshi-cp-dot', enter);
      map.off('mouseleave', 'paqshi-cp-dot', leave);
    };
  }, [mapReady]);

  // ── PAQSHI feed-merge (R2-16): render Paqshi's own intel layers, driven by
  // its /ui/layers manifest (nuclear sites, refineries, LNG, ports, cables,
  // ACLED conflict, FIRMS fires, drought, conflict zones, …). Self-contained:
  // lazily adds a source + layer per activated id and toggles visibility.
  const pqManifestRef = useRef<Record<string, any>>({});
  const pqFetchedRef = useRef<Set<string>>(new Set());
  useEffect(() => {
    if (!mapReady) return;
    const map = mapRef.current;
    if (!map) return;
    ...
    const pqLayerIds = (l: any): string[] =>
      l.render === 'polygon' ? [`pq-${l.id}-fill`, `pq-${l.id}-outline`]
        : l.render === 'line' ? [`pq-${l.id}-line`]
        : [`pq-${l.id}-pt`];

    const addPqLayer = (l: any) => {
      const srcId = `pq-${l.id}`;
      if (map.getSource(srcId)) return;
      map.addSource(srcId, { type: 'geojson', data: EMPTY_FC });
      const color = l.color || '#58a6ff';
      if (l.render === 'line') {
        map.addLayer({ id: `${srcId}-line`, type: 'line', source: srcId,
          paint: { 'line-color': color, 'line-width': 1.4, 'line-opacity': 0.7 } });
      } else if (l.render === 'polygon') {
        ...
        map.addLayer({ id: `${srcId}-pt`, type: 'circle', source: srcId,
          paint: {
            'circle-radius': ['interpolate', ['linear'], ['zoom'], 1, 2.5, 5, 4, 10, 6.5],
            'circle-color': color, 'circle-opacity': 0.85,
            'circle-stroke-width': 0.7, 'circle-stroke-color': 'rgba(0,0,0,0.55)',
          } });
        // generic popup — show the feature's name/label + a couple of props
        const clickId = `${srcId}-pt`;
        const onPqClick = (e: any) => {
          const f = e.features?.[0]; if (!f) return;
          const p = f.properties || {};
          const title = p.name || p.label || p.title || p.place || l.label;
          const rows = Object.entries(p)
            .filter(([k]) => !/^_/.test(k) && !['name', 'label', 'title'].includes(k))
            .slice(0, 5)
            .map(([k, v]) => `<div style="display:flex;justify-content:space-between;gap:12px;font-size:11px;margin:2px 0"><span style="color:#8b949e">${k}</span><span style="color:#e6edf3;text-align:right">${String(v).slice(0, 40)}</span></div>`)
            .join('');
          new maplibregl.Popup({ closeButton: true, maxWidth: '300px' })
            .setLngLat(e.lngLat)
            .setHTML(`<div style="font-family:ui-monospace,monospace;min-width:180px;background:rgba(10,12,22,0.96);border:1px solid ${color}55;border-left:3px solid ${color};border-radius:8px;padding:11px 13px;box-shadow:0 8px 30px rgba(0,0,0,0.6)">
              <div style="font-size:9px;letter-spacing:.16em;color:#58a6ff;text-transform:uppercase;margin-bottom:4px">PAQSHI · ${l.label}</div>
              <div style="font-size:13px;font-weight:700;margin-bottom:6px;color:#fff">${title}</div>${rows}</div>`)
            .addTo(map);
        };
        map.on('click', clickId, onPqClick);
      }
    };

    const sync = async () => {
      if (!Object.keys(pqManifestRef.current).length) {
        try {
          const r = await fetch('/api/paqshi/layers/manifest');
          const d = await r.json();
          (d.layers || []).forEach((l: any) => { pqManifestRef.current[l.id] = l; });
        } catch { /* backend down — nothing to render */ }
      }
      if (cancelled) return;
      const active = new Set(paqshiDataLayers);
      for (const id of active) {
        const l = pqManifestRef.current[id];
        if (!l) continue;
        addPqLayer(l);
        if (!pqFetchedRef.current.has(id)) {
          pqFetchedRef.current.add(id);
          let url = `/api/paqshi/layers/${id}`;
          if (l.needsBbox) {
          ...
          try {
            ...
            if (cancelled) return;
            const src = map.getSource(`pq-${id}`) as any;
            if (src) src.setData(fc && fc.type === 'FeatureCollection' ? fc : { type: 'FeatureCollection', features: fc?.features || [] });
          } catch { /* leave empty */ }
        }
      }
      // visibility for every known layer
      Object.values(pqManifestRef.current).forEach((l: any) => {
        const vis = active.has(l.id) ? 'visible' : 'none';
        pqLayerIds(l).forEach((lid) => { if (map.getLayer(lid)) map.setLayoutProperty(lid, 'visibility', vis); });
      });
    };
    sync();
    return () => { cancelled = true; };
  }, [mapReady, paqshiDataLayers]);
