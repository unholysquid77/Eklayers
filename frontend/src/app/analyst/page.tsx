...
export default function AIAnalystPage() {
  ...
  return (
    <div className="min-h-screen bg-[#040806] text-white p-4 sm:p-6 font-mono selection:bg-[#00ff88]/30 selection:text-[#00ff88]">
      {/* Header Banner */}
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#143a22] pb-5">
        <div>
          <div className="flex items-center gap-2">
            <span className="flex h-7 w-7 items-center justify-center rounded-lg border border-[#00ff88]/40 bg-[#00ff88]/15 text-[#00ff88]">
              <Sparkles className="h-4 w-4 animate-pulse" />
            </span>
            <h1 className="text-xl font-black tracking-wider text-white">SARVADARSHI AI ANALYST</h1>
            <span className="rounded border border-[#00ff88]/40 bg-[#00ff88]/20 px-2 py-0.5 text-[10px] font-bold text-[#00ff88]">
      ...
      <div className="mb-6 grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {PRESET_QUERIES.map((preset, idx) => (
          <button
            ...
            className="flex flex-col items-start rounded-lg border border-[#143a22] bg-[#07140b]/70 p-3 text-left transition-all hover:border-[#00ff88]/50 hover:bg-[#0c2214] group"
          >
            <div className="flex items-center justify-between w-full mb-1">
              <span className="text-[9px] font-bold uppercase tracking-wider text-[#00ff88]">
                {preset.category}
              </span>
              <ArrowRight className="h-3 w-3 text-[#87a894] transition-transform group-hover:translate-x-1 group-hover:text-[#00ff88]" />
            </div>
            <div className="text-xs font-semibold text-white group-hover:text-[#00ff88]">
              {preset.label}
            </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        ...
        <div className="space-y-4 lg:col-span-4">
          {/* Active Query Box */}
          <div className="rounded-xl border border-[#143a22] bg-[#07140b] p-4 shadow-lg">
            <label className="mb-2 block text-xs font-bold uppercase tracking-wider text-[#00ff88]">
            ...
            <div className="relative">
              <textarea
                ...
                rows={4}
                className="w-full resize-none rounded-lg border border-[#143a22] bg-[#040806] p-3 text-xs text-white placeholder-[#87a894]/50 focus:border-[#00ff88] focus:outline-none focus:ring-1 focus:ring-[#00ff88]"
              />
              <button
                onClick={() => handleExecuteQuery(inputQuery)}
                disabled={loading || !inputQuery.trim()}
                className="absolute bottom-3 right-3 flex items-center gap-1.5 rounded bg-[#00ff88] px-3 py-1.5 text-xs font-bold text-black transition-all hover:bg-[#44ffa2] disabled:opacity-40 disabled:cursor-not-allowed"
              >
                {loading ? (
            ...
            <p className="mt-2 text-[10px] text-[#87a894]">
              Press <kbd className="rounded border border-[#143a22] bg-black/50 px-1 py-0.5 text-white">Enter</kbd> to analyze. Synthesizes Bayesian graphs with deterministic BOM exposures.
            </p>
          </div>

          {/* Query Session History */}
          <div className="rounded-xl border border-[#143a22] bg-[#07140b] p-4">
            <div className="mb-3 flex items-center justify-between">
              <span className="flex items-center gap-1.5 text-xs font-bold text-[#87a894]">
                <History className="h-3.5 w-3.5 text-[#00ff88]" />
                SESSION INQUIRY LOG
              </span>
            ...
            <div className="space-y-2">
              {history.map((h) => (
                <button
                  ...
                  className="w-full rounded-lg border border-[#143a22] bg-[#040806] p-2.5 text-left transition-colors hover:border-[#00ff88]/40 hover:bg-[#0c2214]"
                >
        ...
        <div className="space-y-6 lg:col-span-8">
          {loading ? (
            <div className="flex h-96 flex-col items-center justify-center rounded-xl border border-[#143a22] bg-[#07140b] p-8 text-center">
              <span className="h-10 w-10 animate-spin rounded-full border-2 border-[#00ff88] border-t-transparent mb-4" />
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-xl border border-[#00ff88]/40 bg-[#07140b] p-6 shadow-xl relative overflow-hidden">
                <div className="absolute right-0 top-0 h-24 w-24 bg-[#00ff88]/5 blur-2xl pointer-events-none" />
                <div className="flex items-center justify-between border-b border-[#143a22] pb-3 mb-4">
                  <div className="flex items-center gap-2">
                    <span className="rounded border border-[#00ff88]/40 bg-[#00ff88]/15 px-2 py-0.5 text-[10px] font-bold text-[#00ff88]">
                      GROUNDED SYNTHESIS
                    </span>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="rounded-lg border border-[#143a22] bg-[#07140b] p-3 text-center">
                ...
                <div className="rounded-lg border border-[#143a22] bg-[#07140b] p-3 text-center">
                  ...
                  <div className="mt-1 text-lg font-black text-[#00ff88]">
              ...
              {activeResponse.drivers && activeResponse.drivers.length > 0 && (
                <div className="rounded-xl border border-[#143a22] bg-[#07140b] p-5">
                  <div className="mb-3 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                  ...
                  <div className="space-y-2">
                    {activeResponse.drivers.map((driver, idx) => (
                      <div
                        key={idx}
                        className="flex items-start gap-3 rounded-lg border border-[#143a22] bg-[#040806] p-3 text-xs"
                      >
                        <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#00ff88]/10 text-[10px] font-bold text-[#00ff88]">
                          {idx + 1}
                        </span>
              ...
              {activeResponse.recommended_action && (
                <div className="rounded-xl border border-[#00ff88]/30 bg-[#00ff88]/5 p-5 space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="rounded bg-[#00ff88]/20 px-1.5 py-0.5 text-[9px] text-[#00ff88]">
                        [SIMULATED]
                      </span>
                    </div>
                    <Link
                  ...
                  <div className="rounded-lg border border-[#143a22] bg-[#040806] p-3">
                    <div className="text-xs font-bold text-white mb-1">
                      {activeResponse.recommended_action}
                    </div>
                    {activeResponse.expected_effect && (
                      <div className="border-t border-[#143a22] mt-2 pt-2 text-[11px] text-[#87a894]">
                        <strong className="text-[#44ffa2]">Expected Outcome:</strong> {activeResponse.expected_effect}
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                    {activeResponse.citations.map((cit, idx) => (
                      <Link
                        ...
                        className="flex items-center justify-between rounded-lg border border-[#143a22] bg-[#040806] p-2.5 transition-colors hover:border-[#00ff88]/50 hover:bg-[#0c2214] group"
                      >
                        <div className="min-w-0">
                          <div className="text-[9px] uppercase text-[#00ff88]">{cit.entity_kind}</div>
                          <div className="truncate text-xs font-bold text-white group-hover:text-[#00ff88]">
                            {cit.label}
                          </div>
                        </div>
                        <ExternalLink className="h-3 w-3 shrink-0 text-[#87a894] group-hover:text-[#00ff88]" />
                      </Link>
                    ))}
                  </div>
                </div>
